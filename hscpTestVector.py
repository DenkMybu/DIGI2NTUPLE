...
if __name__ == "__main__":
    ...
    NB_iso_below_vs_nb_hscp_tight = TH2D("nb_below_iso_vs_nb_clean_hscp","nb seed < iso 0.7 vs nb tight hscp", 5,0,5,10,0,10)
    NB_iso_below_vs_nb_hscp_tight.GetXaxis().SetTitle("# seed IsoVar < 0.7")
    NB_iso_below_vs_nb_hscp_tight.GetYaxis().SetTitle("# clean HSCP")



    NB_iso_below_per_event = TH1F("nb_below_iso_per_event","nb seed < iso 0.7 per event", 7,0,6)
    ...
    Number_HSCP_per_event = TH1F("nb_HSCP_per_event","nb HSCP per event",15,0,15)
    Number_HSCP_cleaned_per_event = TH1F("nb_HSCP_per_event_post_cuts","nb HSCP per event after cuts on iso and pt",15,0,15)
    ...
    for eventnr,event in enumerate(events):#events
        ...
        Number_HSCP_cleaned_per_event.Fill(len(HSCPVector))
                ...
                if hscparticle[i].trackRef().isNonnull():
                    ...
                    if track.pt() > 50:
                        ...
                        if mindreg < 0.3 and iso_got.Get_TK_SumEt() < 50 and (iso_got.Get_ECAL_Energy() + iso_got.Get_HCAL_Energy())/track.p() < 0.3:
                            ...
                            id_list_hscp_iso,nb_all_ngh_hscp_iso,nb_ngh_below_hscp_iso,nb_ngh_above_hscp_iso,sum_all_ngh_below_hscp_iso = Create3by3MatrixLoop(CaloVector,idx_phi_hscp_iso,idx_eta_hscp_iso,'both',min_mip)
                            sq_oi_hscp = 5
        ...
        if CaloVectorAll:
            nb_non_phys = 0
            nb_below_iso, nb_below_iso_05 = 0,0
            for l in range(len(CaloVectorAll)-1,0,-1):
                ...
                if CaloVectorAll[l][6] > 0:
                    if CaloVectorAll[l][5] > 0.6 and CaloVectorAll[l][6] > 2.5 and (CaloVectorAll[l][5] / CaloVectorAll[l][6]) < 0.3 and (CaloVectorAll[l][5] + CaloVectorAll[l][6]) < 15:
                        ...
                        id_list,nb_all_ngh,nb_ngh_below,nb_ngh_above,sum_all_ngh_below = Create3by3MatrixLoop(CaloVectorAll,idx_phi,idx_eta,'both',min_mip)
                        sq_oi = 5
                        ...
                        if cpt:
                            if nb_ngh_below != 8:
                                Iso_var = sum_all_ngh_below/(8-nb_ngh_below)
                                ...
                                if Iso_var < 0.7:
                                    min_dr_end = 999999
                                    nb_below_iso+=1
                                    for k in range(hscparticle.size()):
                                        if hscparticle[k].trackRef().isNonnull():
                                            track_end = hscparticle[k].trackRef()
                                            dr_end = deltaR(deltaR2(new_eta,new_phi,track_end.eta(),track_end.phi()))
                                            if dr_end < min_dr_end:
                                                min_dr_end = dr_end
    
                                    dr_min_iso_hscp_tower.Fill(min_dr_end)
            ...
            NB_iso_below_per_event.Fill(nb_below_iso)

            NB_iso_below_vs_nb_hscp_tight.Fill(nb_below_iso,len(HSCPVector))
    ...
    Eff_Nseeds_over_nseedshscp.Fill((tot_num_eff/tot_denom_eff)*100)
    hfile.Write()
